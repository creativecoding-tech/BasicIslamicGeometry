#include "SpinLeftAnimation.h"
#include <cmath>

// Constructor
// maxAngle: Amplitudo maksimum spin ke kiri (derajat). Default 90 derajat.
SpinLeftAnimation::SpinLeftAnimation(float maxAngle)
...
// Update spin: Osilasi ke kiri lalu kembali ke 0
void SpinLeftAnimation::update(float deltaTime) {
  if (isComplete())
    return;
...
float SpinLeftAnimation::getAngle() const { return currentAngle; }

void SpinLeftAnimation::setPauseDuration(float duration) {
  pauseDuration = duration;
}

float SpinLeftAnimation::getPauseDuration() const {
  return pauseDuration;
}

void SpinLeftAnimation::reset() {
