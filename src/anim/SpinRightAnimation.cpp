#include "SpinRightAnimation.h"
#include <cmath>

// Constructor
// maxAngle: Amplitudo maksimum spin ke kanan (derajat). Default 90 derajat.
SpinRightAnimation::SpinRightAnimation(float maxAngle)
    : AbstractAnimation(1.0f), currentAngle(0.0f), maxAngle(maxAngle),
      time(0.0f), pauseDuration(0.0f), pauseTimer(0.0f), isPaused(false),
      phase(SpinPhase::FORWARD) {
  // speed (multiplier) diatur melalui setSpeedMultiplier setelah pembuatan
}

// Update spin: Osilasi ke kanan lalu kembali ke 0
void SpinRightAnimation::update(float deltaTime) {
  if (isComplete())
    return;
...
float SpinRightAnimation::getAngle() const { return currentAngle; }

void SpinRightAnimation::setPauseDuration(float duration) {
  pauseDuration = duration;
}

float SpinRightAnimation::getPauseDuration() const {
  return pauseDuration;
}

void SpinRightAnimation::reset() {
