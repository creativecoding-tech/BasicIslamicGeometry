...
//--------------------------------------------------------------
void SacredGeometry::focusWindow() { focusRequested = true; }

//--------------------------------------------------------------
void SacredGeometry::showWindow() { windowOpen = true; }
  ...
  if (ImGui::Begin("Sacred Geometry", &windowOpen, ImGuiWindowFlags_None)) {
    if (app->currentTemplate) {
      ...
      ImGui::TextColored(ImVec4(0.3f, 0.6f, 1.0f, 1.0f), "%s",
                         app->currentTemplate->getName().c_str());
    ...
    ImGui::SetNextItemWidth(150.0f);
    if (app->currentTemplate &&
        ImGui::SliderFloat("Line Width", &app->currentTemplate->lineWidth, 0,
                           4)) {
      app->updateLineWidth();
    }
    ImGui::Separator();
    ...
    if (isTemplateDrawn) {
      ...
      if (ImGui::Button("Parallel")) {
        ...
        if (app->currentTemplate && app->currentTemplate->getShapes().empty()) {
          // Enable semua draw settings (SacredGeometry INDEPENDENT dari
          // Playground!)
