    ...
    if (ImGui::CollapsingHeader("Polygon", ImGuiTreeNodeFlags_DefaultOpen)) {
      ...
      if (ImGui::BeginTable("SpecialPolygonAnimationTable", 3,
        ...
        for (int i = 0; i < app->loadedFilePolygonCount; ++i) {
          ...
          ImGui::RadioButton("Spin Left", &specialPolygonAnimations[i], 3);
          ImGui::RadioButton("Spin Right", &specialPolygonAnimations[i], 4);

          ImGui::PopID();

          // Column 2: Angle Slider (untuk Orbit Left/Right & Spin Left/Right) ⭐ NEW
  ...
  if (ImGui::BeginPopup("TessellationPopup")) {
    ...
    if (tessellationDraw == 1) {
      ...
      if (playMode == 0) { // 0 = Parallel
        ...
        // Template Parallel Mode dengan table 2 kolom, 2 row (TANPA border)
        if (ImGui::BeginTable("TemplateParallelTable", 2, ImGuiTableFlags_None)) {
          // Setup column - sama rata
          ImGui::TableSetupColumn("Col1", ImGuiTableColumnFlags_WidthStretch);
          ImGui::TableSetupColumn("Col2", ImGuiTableColumnFlags_WidthStretch);

          // Row 1: Synchronous (kiri) | Radial Expansion (kanan)
          ImGui::TableNextRow();
          ImGui::TableSetColumnIndex(0);
