...
void ofApp::keyPressed(int key){
	...
	if (key == OF_KEY_BACKSPACE) {
		...
		// Kalau tidak ada polygon dan customLine terselect, toggle CartesianAxes (shape pertama di template)
		else {
			if (!templateShapes.empty()) {
				AbstractShape* cartesianAxes = templateShapes[0].get();  // CartesianAxes selalu index 0
