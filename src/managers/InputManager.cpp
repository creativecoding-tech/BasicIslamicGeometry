...
void InputManager::handleMousePressed(int x, int y, int button) {
  ...
  if (button == 2) {
    vec2 adjustedMousePos =
        GeometryUtils::screenToWorld(vec2(x, y), app->canvasTranslation,
    ...
    for (const auto &dot : templateDots) {
      if (GeometryUtils::isMouseOverDot(adjustedMousePos, dot.position,
                                        app->threshold)) {
        app->contextMenu->setHoveredDotPos(dot.position);
        app->contextMenu->setIsUserDotContext(
            false); // Flag bahwa ini dari original dot
    ...
    for (int i = 0; i < app->polygonShapes.size(); i++) {
      if (app->polygonShapes[i].containsPoint(adjustedMousePos)) {
        ...
        if (app->selectionManager.isPolygonSelected(i)) {
          ...
          app->contextMenu->showWindow(ContextMenu::POLYGON_CONTEXT,
                                       vec2(x, y));
          app->imguiVisible = true;
        }
        return;
    ...
    int clickedLineIndex = GeometryUtils::getLineIndexAtPosition(
        adjustedMousePos, app->customLines, app->mouseLineWidth);
    if (clickedLineIndex >= 0 &&
        app->selectionManager.isLineSelected(clickedLineIndex)) {
      // Tampilkan context menu untuk semua customLine (original dan
      // DcustomLine)
    ...
    // Tidak klik di dot, polygon, atau customLine yang terseleksi → tidak
    // munculkan context menu
    return;
  }
  ...
  // CTRL+Click untuk multi-select (userDot PRIORITY lebih tinggi dari
  // customLine)
  if (button == 0 && app->isCtrlPressed) {
    ...
    for (int i = 0; i < app->polygonShapes.size(); i++) {
      if (app->polygonShapes[i].containsPoint(adjustedMousePos)) {
        ...
        app->syncColorFromSelectedObjects(); // Sync global color dari selected
                                             // object
        return;                              // Jangan lanjut ke logic lain
      }
    }
  ...
  if (button == 0) {
    ...
    if (!clickedOnDot) {
      ...
      for (int i = 0; i < app->polygonShapes.size(); i++) {
        if (app->polygonShapes[i].containsPoint(adjustedMousePos)) {
          // Select polygon ini (single select: hapus yang lama, select yang
          // baru)
      ...
      if (!clickedOnPolygon) {
        int lineIndex = GeometryUtils::getLineIndexAtPosition(
            adjustedMousePos, app->customLines, app->mouseLineWidth);
        if (lineIndex >= 0) {
          ...
          app->selectionManager.clearLineSelection();
          app->selectionManager.clearPolygonSelection(); // Deselect polygon
          // Tidak perlu syncColorFromSelectedObjects() karena tidak ada yang
          // selected
        }
      }
    }
...
void InputManager::handleMouseReleased(int x, int y, int button) {
  ...
  // Adjust mouse position dengan inverse transform
  vec2 releasePos = GeometryUtils::screenToWorld(
      vec2(x, y), app->canvasTranslation, app->canvasZoom, app->canvasRotation);
  vector<DotInfo> dots = app->getAllDots();

  // Check jika release di atas valid dot - HANYA bisa selesai di dot
  for (auto &dot : dots) {
    if (GeometryUtils::isMouseOverDot(releasePos, dot.position,
                                      app->threshold)) {
      // Cek apakah line sudah ada (duplicate check)
      if (!GeometryUtils::lineExists(app->startDotPos, dot.position,
                                     app->customLines)) {
        ...
        if (glm::length(app->startDotPos - dot.position) > 1.0f) {
          ...
          std::string newLabel =
              "customLine" + std::to_string(app->customLines.size());
...
void InputManager::handleMouseMoved(int x, int y) {
  ...
  app->mousePos = GeometryUtils::screenToWorld(
      vec2(x, y), app->canvasTranslation, app->canvasZoom, app->canvasRotation);
}

//--------------------------------------------------------------
void InputManager::handleMouseScrolled(int x, int y, float scrollX,
                                       float scrollY) {
    ...
    for (int index : indices) {
      if (index >= 0 && index < app->userDots.size()) {
        ...
        if (isHorizontalDot) {
          ...
          // Scroll ke depan (scrollY positif) = X minus (ke kiri, makin X
          // minus) Scroll ke belakang (scrollY negatif) = X positif (ke kanan)
          float newX = oldPos.x - scrollY * scrollSpeed;

          // Tentukan batas berdasarkan posisi userDot relative terhadap dot
          // original
          if (oldPos.x < lowerBound.x) {
            // DOT LEFT: batas kanan = dot original (tidak boleh ke kanan
            // melewati dot original)
            float maxX =
                lowerBound.x - 1.0f; // Jarak minimum 1 pixel dari dot original
            if (newX > maxX) {
              newX = maxX; // Clamp ke batas kanan
            }
          } else if (oldPos.x > lowerBound.x) {
            // DOT RIGHT: batas kiri = dot original (tidak boleh ke kiri
            // melewati dot original)
            float minX =
                lowerBound.x + 1.0f; // Jarak minimum 1 pixel dari dot original
            if (newX < minX) {
          ...
          if (oldPos.y < lowerBound.y) {
            // DOT ABOVE: batas bawah = dot original (tidak boleh ke bawah
            // melewati dot original)
            float maxY =
                lowerBound.y - 1.0f; // Jarak minimum 1 pixel dari dot original
            if (newY > maxY) {
              newY = maxY; // Clamp ke batas bawah
            }
          } else if (oldPos.y > lowerBound.y) {
            // DOT BELOW: batas atas = dot original (tidak boleh ke atas
            // melewati dot original)
            float minY =
                lowerBound.y + 1.0f; // Jarak minimum 1 pixel dari dot original
            if (newY < minY) {
        ...
        for (auto &line : app->customLines) {
          ...
          for (size_t i = 0; i < points.size(); i++) {
            // Jika point sama dengan posisi lama (dengan toleransi kecil untuk
            // floating point)
        ...
        if (line.getIsDuplicate()) {
          ...
          for (vec2 &point : points) {
            if (lockState == AxisLock::LOCK_X) {
              ...
              // Scroll down (scrollY negatif) = X positif, Y positif (kanan
              // bawah)
              point.x -= scrollY * scrollSpeed;
              point.y -= scrollY * scrollSpeed;
            }
  ...
  if (!app->isCtrlPressed && app->selectionManager.hasSelectedLine()) {
    ...
    for (int lineIndex : selectedLineIndices) {
      if (lineIndex >= 0 && lineIndex < app->customLines.size()) {
        ...
        float newCurve =
            app->customLines[lineIndex].getCurve() + scrollY * curveSpeed;
        undoAction.newCurve =
            newCurve; // Semua lines dapat curve baru yang sama
...
void InputManager::handleKeyPressed(int key) {
  ...
  if (key == '`' || key == '~') {
    ...
    if (app->currentTemplate) {
      app->currentTemplate->labelsVisible =
          !app->currentTemplate->labelsVisible;
  ...
  if (key == OF_KEY_DEL) {
    ...
    if (app->isCtrlPressed && isShiftPressed) {
      // CTRL+SHIFT+DEL: Clean canvas (hapus semua polygon, custom lines, dan
      // hide template shapes)
      app->cleanCanvas();
      ...
      // User harus menggunakan Clean Canvas atau Draw Template di
      // SacredGeometry panel
    }
  }
  ...
  if (key == OF_KEY_BACKSPACE) {
    ...
    // Note: CartesianAxes toggle dihapus karena tidak sesuai dengan Draw Only
    // concept
  }
  ...
  if ((key == 'B' || key == 'b') && ofGetKeyPressed(OF_KEY_SHIFT)) {
    ...
    app->syncColorFromSelectedObjects(); // Sync global color dari first
                                         // selected line
    return;
  }
  ...
  if ((key == 'g' || key == 'G') && !app->isCtrlPressed) {
    ...
    } else if (!app->showSacredGeometry && !app->showPlayground &&
               !app->showUserCustom) {
      // Show all windows
      app->imguiVisible = true;
