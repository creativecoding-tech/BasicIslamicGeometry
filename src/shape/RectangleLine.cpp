...
RectangleLine::RectangleLine(vec2 start, vec2 end, vec2 intersec1,
                             vec2 intersec2, string label1, string label2,
                             float radius)
    : start(start), end(end), intersec1(intersec1), intersec2(intersec2),
      originalStart(start), originalEnd(end), originalIntersec1(intersec1),
      originalIntersec2(intersec2), label1(label1), label2(label2),
...
void RectangleLine::setIntersec1(vec2 intersec1) {
  ...
  this->originalIntersec1 = intersec1;
}

void RectangleLine::setIntersec2(vec2 intersec2) {
  this->intersec2 = intersec2;
  this->originalIntersec2 = intersec2;
}

void RectangleLine::showDot() { dotVisible = true; }

void RectangleLine::hideDot() { dotVisible = false; }

void RectangleLine::showLabel() { labelVisible = true; }

void RectangleLine::hideLabel() { labelVisible = false; }
...
void RectangleLine::setRadius(float r) {
  ...
  if (std::abs(scaleFactor - 1.0f) < 0.0001f) {
    ...
    intersec1 = originalIntersec1;
    intersec2 = originalIntersec2;
  } else {
    start = originalStart * scaleFactor;
    end = originalEnd * scaleFactor;
    intersec1 = originalIntersec1 * scaleFactor;
    intersec2 = originalIntersec2 * scaleFactor;
  }
  radius = r;
