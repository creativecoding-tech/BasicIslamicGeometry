...
#include "DotInfo.h"
OctagramLine::OctagramLine(vec2 start, vec2 end, std::optional<vec2> nextPoint,
                           string label, float radius)
    : start(start), end(end), nextPoint(nextPoint), originalStart(start),
      originalEnd(end), originalNextPoint(nextPoint), label(label),
...
void OctagramLine::setStart(float startX, float startY) {
  start = vec2(startX, startY);
  originalStart = start;
}

void OctagramLine::setEnd(float endX, float endY) {
  end = vec2(endX, endY);
  originalEnd = end;
}

void OctagramLine::showDot() { dotVisible = true; }

void OctagramLine::hideDot() { dotVisible = false; }

void OctagramLine::showLabel() { labelVisible = true; }

void OctagramLine::hideLabel() { labelVisible = false; }
